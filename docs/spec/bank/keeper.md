...
type DenomMetadata struct {
  Name        string   // The name of the denom. Used in referencing throughout the SDK. Example: Ether
	Aliases     []string // Alternative names for a token. No guarantee of non-collisions. Example: ETH
	Decimals    int8     // Number of digits that come after the decimal place. 1 TOKEN = 10^(-N). Important for UI.
	TotalSupply sdk.Int  // The total supply of the token in its smallest unit (i.e. satoshi or wei)
}
```
